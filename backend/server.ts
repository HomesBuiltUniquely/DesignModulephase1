...
const pool = mysql.createPool({
  ...
  password: process.env.DB_PASSWORD || "Root@12",
...
async function getMailLoopCcEmails(extraEmails: Array<string | null | undefined> = [], leadId?: number): Promise<string[]> {
  ...
  if (leadId) {
    ...
    try {
      ...
      const pmEmail = (pmRows as { email: string | null }[])[0]?.email;
      if (pmEmail) {
...
function parseLeadPayloadObject(raw: unknown): Record<string, unknown> {
  try {
    if (raw == null) return {};
...
function readTotalPaidToward10Percent(payload: Record<string, unknown>): number {
...
function salesClosureTenPercentMet(payload: Record<string, unknown>): boolean {
...
function normalizeFinanceSubmissionHistory(
  payload: Record<string, unknown>,
): FinanceSubmissionRecord[] {
  let history: FinanceSubmissionRecord[] = [];
    ...
    history = raw
      .map((item, i) => {
        ...
        const cumulativeTotal =
          parseFiniteNumber(o.cumulativeTotal) ?? parseFiniteNumber(o.cumulative_total) ?? amount;
        const submittedAt =
        ...
        return {
          ...
          paymentReceived:
            typeof o.paymentReceived === "string"
              ? o.paymentReceived
        ...
        } satisfies FinanceSubmissionRecord;
      })
      .filter(Boolean) as FinanceSubmissionRecord[];
  } else {
      ...
      history = legacy
        .map((item, i) => {
          ...
          return {
            ...
            screenshot: i === legacy.length - 1 ? (payload.payment_screenshot as string) || null : null,
            submittedAt: typeof o.at === "string" ? o.at : new Date().toISOString(),
            remainingFor10Percent:
              target != null && target > 0 ? Math.max(0, target - running) : null,
          } satisfies FinanceSubmissionRecord;
        })
        .filter(Boolean) as FinanceSubmissionRecord[];
    } else {
        ...
        history = [
          {
            ...
            paymentReceived:
              typeof payload.payment_received === "string" ? payload.payment_received : null,
            screenshot:
              typeof payload.payment_screenshot === "string" ? payload.payment_screenshot : null,
            submittedAt:
              typeof payload.sales_closure_submitted_at === "string"
                ? payload.sales_closure_submitted_at
                : new Date().toISOString(),
            remainingFor10Percent:
              target != null ? Math.max(0, target - cumulative) : parseFiniteNumber(payload.remaining_for_10_percent),
          },
        ];
      }
    }
...
function reconcileFinanceSubmissionHistory(
  payload: Record<string, unknown>,
  history: FinanceSubmissionRecord[],
): FinanceSubmissionRecord[] {
    ...
    if (
      ...
      rows = [
        ...
        },
        {
          ...
          amount: lastTranche,
          submittedAt:
            typeof payload.sales_closure_payment_added_at === "string"
...
function appendFinanceSubmission(
  payload: Record<string, unknown>,
  entry: Omit<FinanceSubmissionRecord, "id"> & { id?: string },
  ctx?: { previousCumulative?: number; existingPayload?: Record<string, unknown> },
): void {
  const existing = ctx?.existingPayload ?? payload;
  const previousCumulative =
    ctx?.previousCumulative ?? Math.max(0, readTotalPaidToward10Percent(existing) - entry.amount);
  ...
  if (history.length === 0 && previousCumulative > 0 && entry.amount > 0) {
    ...
    history.push({
      ...
      mode:
        typeof existing.mode_of_payment === "string"
          ? existing.mode_of_payment
          : typeof payload.mode_of_payment === "string"
            ? payload.mode_of_payment
            : null,
      paymentReceived:
        typeof existing.payment_received === "string"
          ? existing.payment_received
          : typeof payload.payment_received === "string"
            ? payload.payment_received
            : null,
      screenshot:
        typeof existing.payment_screenshot === "string" ? existing.payment_screenshot : null,
      submittedAt:
        typeof existing.sales_closure_submitted_at === "string"
          ...
          : typeof existing.sales_closure_payment_added_at === "string"
            ...
            : new Date().toISOString(),
      remainingFor10Percent:
        target != null ? Math.max(0, target - previousCumulative) : null,
    });
  }
...
async function persistRepairedFinanceHistory(leadId: number, payload: Record<string, unknown>): Promise<void> {
  const repaired = reconcileFinanceSubmissionHistory(
    payload,
    normalizeFinanceSubmissionHistory(payload),
  );
  const current = payload.finance_submission_history;
  const currentJson = JSON.stringify(current ?? []);
  const repairedJson = JSON.stringify(repaired);
  if (currentJson === repairedJson) return;
...
function applySalesClosurePaymentTotals(
  payload: Record<string, unknown>,
  thisPayment: number,
      ...
      appendFinanceSubmission(payload, {
        ...
        submittedAt:
          typeof payload.sales_closure_submitted_at === "string"
            ...
            : new Date().toISOString(),
        remainingFor10Percent:
          target != null ? Math.max(0, target - cumulative) : null,
      });
    }
  }
  ...
  try {
    ...
    if (emailRoutePath && !isFirstCompletion) {
      ...
      if (emailRoutePath === "DQC1_APPROVAL_DUAL") {
        ...
        try {
          const [rows] = await pool.query(
            `SELECT l.pid, l.project_name as projectName, l.payload, l.assigned_designer_id, l.assigned_project_manager_id,
          ...
          if (row) {
            ...
            const customerName =
              ...
              "Customer";
            const ecName =
              payload.experience_center || payload?.form?.experience_center || formData.experience_center || "";
            const pmEmail = row.pmEmail || null;
            const pmName = row.pmName || null;
            const designerName = row.designerName || "Designer";
...
function mapFinanceSalesClosureQueueRow(
  r: Record<string, unknown>,
  options: { approved: boolean },
): Record<string, unknown> {
  const payloadObj = parseLeadPayloadObject(r.payload);
  const amountPaid =
    readTotalPaidToward10Percent(payloadObj) ||
    parseFiniteNumber(r.amountPaid) ||
    parseFiniteNumber(payloadObj.amount_paid);
  const tenPercentTarget =
    parseFiniteNumber(r.tenPercentTarget) ?? parseFiniteNumber(payloadObj.ten_percent_target);
  let remaining =
    parseFiniteNumber(r.remainingFor10Percent) ??
  ...
  const financeApproved =
    ...
    payloadObj.sales_closure_finance_approved === "true";
  const percentOfQuotation =
    parseFiniteNumber(r.paymentPercentOfQuotation) ??
  ...
  const approvedAt =
    (typeof r.financeApprovedAt === "string" && r.financeApprovedAt.trim()) ||
  ...
  return {
    ...
    customerName:
      (typeof r.customerName === "string" && String(r.customerName).trim()) ||
...
app.get("/api/leads/finance-sales-closure-queue", async (req: Request, res: Response) => {
  try {
    ...
    if (customerName) {
      ...
      conditions.push(
        `(LOWER(project_name) LIKE ? OR LOWER(JSON_UNQUOTE(JSON_EXTRACT(${payloadJson}, '$.customer_name'))) LIKE ?)`,
      );
...
app.get("/api/leads/finance-sales-closure/:leadId", async (req: Request, res: Response) => {
  try {
    const user = await getUserFromSession(req);
    if (!user) return res.status(401).json({ message: "Unauthorized" });
    const role = (user?.role ?? "").toLowerCase();
    ...
    const [rows] = await pool.query(
      `SELECT id, project_name as projectName, project_stage as projectStage, payload, create_at
...
app.post("/api/leads/:id/approve-sales-closure", async (req: Request, res: Response) => {
  try {
    ...
    const [leadRows] = await pool.query(
      `SELECT payload FROM leads WHERE id = ? LIMIT 1`,
      [leadId],
    );
...
app.post("/api/leads/:id/reject-sales-closure", async (req: Request, res: Response) => {
  try {
    ...
    const salesEmail =
      typeof payloadObj.sales_email === "string" ? payloadObj.sales_email : null;
    const salesLeadEmail =
      typeof payloadObj.sales_lead_email === "string" ? payloadObj.sales_lead_email : null;
    const salesSpocEmail =
      typeof payloadObj.sales_spoc_email === "string" ? payloadObj.sales_spoc_email : null;
    const customerName: string =
      (typeof payloadObj.customer_name === "string" && payloadObj.customer_name) ||
      row?.projectName ||
      "Customer";
...
app.put("/api/sales-closure/:id", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const thisPayment =
      parseFiniteNumber(incoming.amount_paid_this_time) ??
      parseFiniteNumber(incoming.amount_paid) ??
      0;
    applySalesClosurePaymentTotals(updatedPayload, thisPayment, previousCumulative);

    const screenshot =
      typeof incoming.payment_screenshot === "string" && incoming.payment_screenshot
    ...
    if (thisPayment > 0 || screenshot) {
      ...
      appendFinanceSubmission(
        updatedPayload,
        {
          amount: thisPayment,
          cumulativeTotal: cumulative,
          mode:
            typeof incoming.mode_of_payment === "string"
              ? incoming.mode_of_payment
              : typeof existingPayload.mode_of_payment === "string"
                ? existingPayload.mode_of_payment
                : null,
          paymentReceived:
            typeof incoming.payment_received === "string"
    ...
    await pool.query(
      `UPDATE leads SET payload = ?, project_stage = 'Pre 10%', update_at = ? WHERE id = ?`,
...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      try {
        const [leadRows] = await pool.query(
          "SELECT pid, project_name as projectName FROM leads WHERE id = ?",
          [leadId]
        );
        const leadRow = (leadRows as any[])[0];
        if (leadRow) {
          ...
          const [financeRows] = await pool.query(
            "SELECT email FROM users WHERE role = 'finance' AND email IS NOT NULL"
          );
          let financeEmails = (financeRows as any[]).map(r => r.email);
          ...
          const ccEmails = (ccRows as any[]).map(r => r.email);
...
app.post("/api/leads/:id/approve-10p-payment", async (req: Request, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      const notifyRow = (notifyRows as any[])[0];
      if (notifyRow) {
...
app.post("/api/leads/:id/dqc-review", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [latestReviewRows] = await pool.query(
      `SELECT verdict FROM lead_dqc_reviews WHERE lead_id = ? ORDER BY id DESC LIMIT 1`,
      [leadId],
    );
    const lastVerdict = (latestReviewRows as any[])[0]?.verdict;

    // Check if DQC 1 approval task is completed
    const [dqc1CompleteRows] = await pool.query(
      `SELECT id FROM lead_task_completions WHERE lead_id = ? AND task_name = 'DQC 1 approval' LIMIT 1`,
      [leadId],
    );
...
app.post("/api/leads/:id/assign-designer", async (req: Request, res: Response) => {
  try {
    ...
    try {
      const [nRows] = await pool.query(
        `SELECT pid, project_name as projectName FROM leads WHERE id = ? LIMIT 1`,
        [leadId],
      );
      const nRow = (nRows as any[])[0];
      if (nRow) {
