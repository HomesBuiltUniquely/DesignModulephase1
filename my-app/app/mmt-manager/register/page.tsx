...
export default function MmtManagerRegisterPage() {
  ...
  useEffect(() => {
    ...
    if (user.role !== 'mmt_manager') router.replace('/');
  }, [user, loading, router]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${API}/api/auth/register-mmt-executive`, {
        method: 'POST',
        headers: {
  ...
  if (user.role !== 'mmt_manager') return null;

  return (
    <div className="min-h-screen bg-slate-100">
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <h1 className="text-xl font-bold text-gray-900">Register MMT Executive</h1>
      ...
      <main className="max-w-md mx-auto p-8">
        <div className="bg-white rounded-2xl shadow border border-gray-200 p-6">
          <p className="text-gray-600 text-sm mb-4">MMT Manager can register MMT (Measurement Team) Executives. They will sign in with the credentials you set. Email must end with @hubinterior.com.</p>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {submitting ? 'Registering…' : 'Register MMT Executive'}
            </button>
          </form>
        </div>
      </main>
