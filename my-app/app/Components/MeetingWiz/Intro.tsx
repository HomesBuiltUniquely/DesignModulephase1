  ...
  return (
    <main className="min-h-screen w-full bg-[#f0f4f8]">
      ...
      <div className="flex flex-col items-center py-4">
        <div className="flex items-center gap-1">
          {Array.from({ length: 5 }).map((_, i) => (
            <div
              key={i}
              className={`h-1 rounded-full w-8 ${i < 1 ? "bg-[var(--brand-primary)]" : "bg-gray-300"}`}
            />
          ))}
        </div>
      ...
      >
        ...
        <button
          onClick={onNext}
          className="inline-flex items-center gap-3 rounded-lg bg-[var(--brand-primary)] px-6 py-3 text-sm font-bold uppercase tracking-wider text-white transition hover:opacity-90"
        >
          Next: About Hub
