...
function getPhaseBucket(p: LeadshipTypes): string {
  const fromMilestone = getPhaseFromMilestone(
    p.currentMilestoneIndex,
    p.currentMilestoneProgress,
  );
...
function getStatusFromBucket(bucket: string): {
  label: string;
  className: string;
} {
  if (bucket === "Pre 10%")
    return { label: "DELAYED", className: "bg-red-100 text-red-800" };
  if (bucket === "10-20%")
    return { label: "UNDER QC", className: "bg-purple-100 text-purple-800" };
  if (bucket === "20-60%")
    return { label: "READY PROD.", className: "bg-green-100 text-green-800" };
...
export default function Dashboard() {
  ...
  const isMmtUser = ["mmt_manager", "mmt_executive"].includes(
    (user?.role || "").toLowerCase(),
  );
  const isDqcUser = ["dqc_manager", "dqe"].includes(
    (user?.role || "").toLowerCase(),
  );
      ...
      <div className="p-6 max-w-2xl mx-auto">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 text-center">
          ...
          <p className="text-gray-600 mb-6">
            As finance, you can only upload payment screenshots and approve 10%
            and 40% payments. You do not have access to the full project list or
            lead details.
          </p>
  ...
  const [assignableDesigners, setAssignableDesigners] = useState<
    AssignableDesigner[]
  >([]);
  const [selectedLeadIds, setSelectedLeadIds] = useState<number[]>([]);
  const [bulkDesignerId, setBulkDesignerId] = useState<number | "">("");
  const [bulkAssignLoading, setBulkAssignLoading] = useState(false);
  const [bulkAssignMessage, setBulkAssignMessage] = useState<string | null>(
    null,
  );
  const [singleAssignByLead, setSingleAssignByLead] = useState<
    Record<number, number | "">
  >({});
  const [singleAssignLoadingLeadId, setSingleAssignLoadingLeadId] = useState<
    number | null
  >(null);
  ...
  const milestoneFiltered = milestoneFilter
    ? searchFiltered.filter(
        (p) => (p.currentMilestoneName ?? "") === milestoneFilter,
      )
    : searchFiltered;
  ...
  const paginatedProjects = milestoneFiltered.slice(
  ...
  const stats = {
    ...
    pre10: milestoneFiltered.filter((p) => getPhaseBucket(p) === "Pre 10%")
      .length,
    bucket1020: milestoneFiltered.filter((p) => getPhaseBucket(p) === "10-20%")
      .length,
    bucket2060: milestoneFiltered.filter((p) => getPhaseBucket(p) === "20-60%")
      .length,
  };
  ...
  const assignSingleLead = async (leadId: number) => {
    ...
    try {
      ...
      setBulkAssignMessage(
        `Lead ${leadId} assigned to ${data?.designerName || "selected designer"}.`,
      );
    } catch (err) {
      setBulkAssignMessage(
  ...
  const runExcelPreview = async () => {
    ...
    try {
      ...
      setImportMessage(
        `Preview loaded: ${Number(data.rowCount || 0)} rows found.`,
      );
    } catch (err: unknown) {
    ...
    try {
      const res = await fetch(`${API}/api/leads/import-excel/commit`, {
        ...
        body: JSON.stringify({
          token: previewToken,
          mappings,
          defaultDesignerId: importDesignerId,
        }),
      });
      const data = await res.json().catch(() => null);
      if (!res.ok) throw new Error(data?.message || "Import failed");
      setImportMessage(
        `Import complete. Imported ${data.imported}/${data.totalRows}. Failed: ${data.failed}.`,
      );
  ...
  const renderDqcRow = (
    ...
    >
      ...
      <div className="xl:text-lg xl:font-semibold xl:text-left">
        {row.projectName}
      </div>
      <div className="xl:text-lg xl:font-semibold xl:text-center">
        {getStatusDisplay(row.projectStage)}
      </div>
      <div
        className="xl:text-center"
        onClick={(e) => {
          e.stopPropagation();
          handleRouter(row.id, stage);
        }}
      >
      ...
      <div
        className={`xl:text-center xl:font-semibold xl:text-sm ${row.dqcStatus === "Approved DQC" ? "xl:text-green-700" : "xl:text-amber-700"}`}
      >
        {row.dqcStatus}
      </div>
    </div>
  ...
  const renderContent = () => {
    if (isDqcUser) {
      ...
      return (
        <div className="xl:space-y-8">
          <div>
            <h3 className="xl:text-base xl:font-bold xl:mb-3 xl:text-gray-800">
              DQC 1 Approval
            </h3>
            {dqc1List.length === 0 ? (
              <p className="xl:text-sm xl:text-gray-500 xl:p-4">
                No leads pending DQC 1 approval.
              </p>
            ) : (
          ...
          <div>
            ...
            {dqc2List.length === 0 ? (
              <p className="xl:text-sm xl:text-gray-500 xl:p-4">
                No leads pending DQC 2 approval.
              </p>
            ) : (
        ...
        <div>
          {list.map((arr1) => (
            ...
            >
              ...
              <div className="xl:text-lg xl:font-semibold xl:text-center">
                {getStatusDisplay(arr1.projectStage)}
              </div>
              <div
                className="xl:text-center"
                onClick={(e) => onUploadClick(e, arr1.id)}
              >
                <button
                  type="button"
    ...
    const deduped = paginatedProjects.filter(
      (p, i, arr) => arr.findIndex((x) => x.id === p.id) === i,
    );
    return (
            ...
            <table className="w-full min-w-[920px]">
                  ...
                  {canImportLeads && (
                    <th className="py-3 px-3">
                      <input
                        ...
                        onChange={() =>
                          toggleSelectAllVisible(deduped.map((d) => d.id))
                        }
                      />
                    </th>
                  )}
              ...
              <tbody>
                {deduped.map((row) => {
                  ...
                  const statusInfo =
                    row.currentMilestoneIndex != null ||
                    row.currentMilestoneName
                      ? getStatusFromMilestone(
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-5 text-sm text-gray-700">
                        {row.currentMilestoneName ?? bucket}
                      </td>
                      <td className="py-3 px-5">
                        <div className="flex items-center gap-2">
                      ...
                      <td className="py-3 px-5">
                        <span
                          className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold ${statusInfo.className}`}
                        >
                          {statusInfo.label}
                        </span>
                      </td>
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {canImportLeads && (
                            <>
                              ...
                              >
                                {singleAssignLoadingLeadId === row.id
                                  ? "..."
                                  : "Assign"}
                              </button>
                            </>
        ...
        <div className="xl:grid xl:grid-cols-5 xl:gap-4">
          <div className=" xl:grid-cols-1 xl:h-screen border-r border-gray-300 xl:pt-4 xl:pl-2">
            {!isDqcUser && (
              <>
                ...
                >
                  <div className="xl:text-lg xl:font-semibold xl:pl-2 xl:pt-4">
                    My WorkSpace
                  </div>
                  <div>
                    <svg
                ...
                {isDropdownOpen && (
                  <div className="xl:transition-all xl:duration-200">
                    <p className="xl:pl-2 xl:pt-2 xl:pb-1 text-xs font-bold text-gray-500 uppercase tracking-wide">
                      Design Phases
                    </p>
          ...
          <div className="xl:col-span-4 min-w-0 flex-1 w-full overflow-auto">
            {!isDqcUser && !isMmtUser ? (
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <div className="relative" ref={filterDropdownRef}>
                      ...
                      >
                        ...
                        {milestoneFilter ? (
                          <span
                            className="text-blue-600 font-semibold truncate max-w-[120px]"
                            title={
                              MILESTONE_FILTER_OPTIONS.find(
                                (o) => o.value === milestoneFilter,
                              )?.label
                            }
                          >
                            {
                      ...
                      {filterDropdownOpen && (
                        <div className="absolute right-0 top-full mt-1 z-50 min-w-[200px] py-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-64 overflow-y-auto">
                          <p className="px-3 py-2 text-xs font-semibold text-gray-500 uppercase border-b border-gray-100">
                            By milestone
                          </p>
                ...
                {canImportLeads && (
                  <div className="rounded-xl border border-emerald-100 bg-emerald-50/30 p-4 space-y-3">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      >
                        <option value="">
                          Select designer for bulk assign
                        </option>
                        {assignableDesigners.map((d) => (
                          <option key={d.id} value={d.id}>
                            {d.name} ({d.role.replace("_", " ")})
              ...
              <>
                <div
                  className={`xl:grid xl:gap-4 xl:min-w-287.5 xl:p-4 xl:m-2 xl:border xl:border-gray-300 xl:rounded-lg xl:shadow-md xl:bg-black xl:text-white xl:text-lg xl:font-bold xl:items-center ${isDqcUser ? "xl:grid-cols-5" : "xl:grid-cols-4"}`}
                >
                  <div className="xl:text-center">ID</div>
                  <div className="xl:text-left">
                    {isDqcUser ? "Name" : "Project Name"}
                  </div>
