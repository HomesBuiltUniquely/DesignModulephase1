...
export default function DqcManagerRegisterPage() {
  ...
  useEffect(() => {
    ...
    if (user.role !== 'dqc_manager') router.replace('/');
  }, [user, loading, router]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${API}/api/auth/register-dqe`, {
        method: 'POST',
        headers: {
      ...
      setMessage({ type: 'success', text: `DQE (Design Quality Executive) created: ${data.user?.email}` });
      setEmail('');
      setPassword('');
      setName('');
      setPhone('');
  ...
  if (user.role !== 'dqc_manager') return null;
