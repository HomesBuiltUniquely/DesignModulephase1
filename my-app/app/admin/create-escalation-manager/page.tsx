...
export default function AdminCreateEscalationManagerPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${API}/api/auth/create-escalation-manager`, {
        method: 'POST',
        headers: {
      ...
      if (!res.ok) {
        setMessage({ type: 'error', text: data.message || 'Failed to create Escalation Manager' });
        return;
      }
      setMessage({ type: 'success', text: `Escalation Manager created: ${data.user?.email}` });
      setEmail('');
      setPassword('');
      setName('');
      setPhone('');
    ...
    <div className="min-h-screen bg-slate-100">
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <h1 className="text-xl font-bold text-gray-900">Admin — Create Escalation Manager</h1>
      ...
      <main className="max-w-md mx-auto p-8">
        <div className="bg-white rounded-2xl shadow border border-gray-200 p-6">
          <p className="text-gray-600 text-sm mb-4">Only Admin can create Escalation Manager logins. They can sign in and handle escalations. Email must end with @hubinterior.com.</p>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full border border-gray-300 rounded-lg px-4 py-2.5"
                placeholder="Escalation Manager name"
              />
            </div>
            <div>
            ...
            >
              {submitting ? 'Creating…' : 'Create Escalation Manager'}
            </button>
          </form>
        </div>
      </main>
