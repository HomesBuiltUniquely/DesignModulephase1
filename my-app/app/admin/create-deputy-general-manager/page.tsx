...
export default function AdminCreateDeputyGeneralManagerPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${API}/api/auth/create-deputy-general-manager`, {
        method: 'POST',
        headers: {
      ...
      if (!res.ok) {
        setMessage({ type: 'error', text: data.message || 'Failed to create Deputy General Manager' });
        return;
      }
      setMessage({ type: 'success', text: `Deputy General Manager created: ${data.user?.email}` });
      setEmail('');
      setPassword('');
      setName('');
      setPhone('');
    ...
    <div className="min-h-screen bg-slate-100">
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <h1 className="text-xl font-bold text-gray-900">Admin — Create Deputy General Manager</h1>
      ...
      <main className="max-w-md mx-auto p-8">
        <div className="bg-white rounded-2xl shadow border border-gray-200 p-6">
          <p className="text-gray-600 text-sm mb-4">Only Admin can create Deputy General Managers. Email must end with @hubinterior.com.</p>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {submitting ? 'Creating…' : 'Create Deputy General Manager'}
            </button>
          </form>
        </div>
      </main>
