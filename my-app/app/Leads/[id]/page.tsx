...
import {
  ...
  getChecklistKeyForTask,
} from "./components/Checklists/checklistRegistry";
...
export default function ProjectDetailPage() {
  ...
  const taskKey = (milestoneIndex: number, taskName: string) =>
  ...
  // context used specifically for the "visit checklist" menu action
  ...
  const [blockedTaskMessage, setBlockedTaskMessage] = useState<string | null>(
    null,
  );
  const [dqc1Verdict, setDqc1Verdict] = useState<
    "approved" | "approved_with_changes" | "rejected" | null
  >(null);
  const [dqc1Remarks, setDqc1Remarks] = useState<QCRemark[]>([
    {
      id: 1,
      priority: "high",
      text: "Dimension Issue: Missing cabinet clearance dimensions on Wall B. The spacing between the refrigerator island and main counter seems insufficient for ADA compliance.",
      pinNumber: 1,
      xPct: 28,
      yPct: 35,
      page: 1,
    },
    {
      id: 2,
      priority: "medium",
      text: "Material Issue: Check backsplash material compatibility with induction heat specs. Quartz might require additional heat shielding behind the range area.",
      pinNumber: 2,
      xPct: 55,
      yPct: 52,
      page: 1,
  ...
  const [dqc1CommentPopup, setDqc1CommentPopup] = useState<{
    xPct: number;
    yPct: number;
    page: number;
    docX?: number;
  ...
  const [dqc1HighlightedPin, setDqc1HighlightedPin] = useState<number | null>(
    null,
  );
  ...
  const [historyEvents, setHistoryEvents] = useState<HistoryEvent[]>(() => [
    {
      ...
      description:
        "DQC 1 approval completed. Design QC review submitted with verdict and remarks.",
      ...
      details: {
        ...
        remarks: [
          {
            priority: "high",
            text: "Dimension Issue: Missing cabinet clearance dimensions on Wall B.",
          },
          {
            priority: "medium",
            text: "Material Issue: Check backsplash material compatibility with induction heat specs.",
          },
        ],
      },
    },
    {
      ...
      description:
        "Task 1 status: On time → Delayed. System bottleneck detected in backend API response times.",
    ...
    },
    {
      ...
      details: {
        kind: "note",
        noteText:
          "Still waiting for the final site measurement data from the vendor before we can commit to the foundation phase.",
      },
    },
    {
      id: "ev-4",
      ...
      description:
        "Owner changed: Alex → Saranya. Handover of Phase 1 responsibilities completed.",
      user: { name: "System" },
    },
    {
      id: "ev-5",
      ...
      details: {
        kind: "file_upload",
        fileName: "Final_Blueprint_V2.pdf",
        size: "4.2 MB",
    ...
    },
    {
      ...
      details: {
        kind: "mom",
        minutes:
          "Finalised layout and material choices. Client approved backsplash option B. Next: DQC 1 submission by EOW.",
        referenceFiles: [
          { name: "Meeting_Notes.pdf", size: "120 KB" },
          { name: "Layout_v3.dwg", size: "2.1 MB" },
        ],
      },
    },
  ]);
  ...
  const projects: LeadshipTypes[] = [
    {
      id: 1,
      pid: "P001",
      projectName: "Bharath D",
      projectStage: "Active",
      createAt: "2023-01-01T10:00:00.000Z",
      updateAt: "2023-01-02T14:30:00.000Z",
    },
    {
      id: 2,
      pid: "P002",
      projectName: "Shivram",
      projectStage: "Active",
      createAt: "2023-02-01T09:15:00.000Z",
      updateAt: "2023-02-02T11:00:00.000Z",
    },
    {
      id: 3,
      pid: "P003",
      projectName: "Gokulnath",
      projectStage: "Inactive",
      createAt: "2023-03-01T08:00:00.000Z",
      updateAt: "2023-03-02T16:45:00.000Z",
    },
    {
      id: 4,
      pid: "P004",
      ...
      updateAt: "2024-02-01T09:00:00.000Z",
    },
    {
      id: 5,
      pid: "P005",
      ...
      projectStage: "10-20%",
      createAt: "2024-03-10T14:00:00.000Z",
      updateAt: "2024-04-05T11:20:00.000Z",
    },
  ];
  ...
  const handleImageAdding = () => {
    const maxId =
      image.length > 0 ? Math.max(...image.map((img) => img.id)) : 0;
  ...
  const openTaskPopup = (milestoneIndex: number, taskName: string) => {
    ...
    if (milestone) {
      ...
      if (taskName === "DQC 1 approval" || taskName === "Design sign off")
        setDqc1Verdict(null);
    }
  };

  // called from MilestonesCard when user selects "Visit checklist" from three‑dot menu
  const openVisitChecklist = (milestoneIndex: number, taskName: string) => {
    // close any existing task popup
    setPopupContext(null);
    setChecklistContext({ milestoneIndex, taskName });
  };
  ...
  const closePopup = () => {
    setChecklistContext(null); // ensure checklist also reset
    setPopupContext(null);
    setDqc1Verdict(null);
    setDqc1Remarks([
      {
  ...
  const removeDqc1Remark = (id: number) =>
    setDqc1Remarks((prev) => prev.filter((r) => r.id !== id));
  ...
  const focusRemarkInPdf = (r: QCRemark) => {
    ...
    requestAnimationFrame(() => {
      dqc1PdfViewportRef.current?.scrollIntoView({
        behavior: "smooth",
        block: "center",
      });
      setTimeout(() => {
        ...
        const pinCenterX =
          viewport.offsetLeft + (r.xPct / 100) * viewport.offsetWidth;
        const pinCenterY =
          viewport.offsetTop + (r.yPct / 100) * viewport.offsetHeight;
        scrollEl.scrollTo({
  ...
  const removeMomFile = (index: number) =>
    setMomReferenceFiles((prev) => prev.filter((_, i) => i !== index));
  ...
  const getTaskStatus = (
    ...
    const firstIncompleteIndex = taskList.findIndex(
      (t) => !completedTaskKeys.includes(taskKey(milestoneIndex, t)),
    );
  ...
  const checklistKey = checklistContext
    ? getChecklistKeyForTask(
        checklistContext.milestoneIndex,
        checklistContext.taskName,
      )
    : null;

  const activeChecklistDefinition = checklistKey
    ? checklistDefinitions[checklistKey]
    : null;
  ...
  return (
    <div className="bg-slate-900 xl:min-h-[900px] 2xl:min-h-[1400px]">
      ...
      {/* --- MAIN GRID --- */}
      <main
        className={`xl:grid xl:grid-cols-7 xl:gap-5 xl:m-4 transition-all duration-300 ${activeCard ? "block" : ""}`}
      >
        <MilestonesCard
          cardClass={getCardClass(
            "milestones",
            "xl:col-span-3 xl:bg-purple-50 xl:h-[70vh] xl:rounded-3xl relative xl:pt-6 xl:pb-6 xl:px-4",
          )}
          ...
          onVisitChecklist={openVisitChecklist}
        ...
        <HistoryCard
          cardClass={getCardClass(
            "history",
            "xl:col-span-2 xl:bg-purple-50 xl:h-[70vh] xl:text-center xl:font-bold xl:pt-8 xl:rounded-3xl text-gray-400 relative",
          )}
        ...
        <div
          className={`xl:col-span-2 xl:h-full xl:text-center xl:font-bold ${activeCard && activeCard !== "files" && activeCard !== "chat" ? "hidden" : ""}`}
        >
          <div className="xl:grid xl:grid-rows-2 xl:h-full xl:gap-4">
            <FilesCard
              cardClass={getCardClass(
                "files",
                "xl:rounded-3xl xl:bg-purple-50 xl:row-span-1 xl:text-center xl:font-bold xl:pt-8 text-gray-400 relative",
              )}
      ...
      {popupContext && (
        <TaskModal context={popupContext} onClose={closePopup}>
          ...
          {popupContext.milestoneIndex === 0 &&
            popupContext.taskName === "D1 for MMT request" && (
              <PopupD1Measurement
                onSubmit={() => {
                  markTaskComplete(0, "D1 for MMT request");
                  closePopup();
                }}
              />
            )}
          {popupContext.milestoneIndex === 0 &&
            popupContext.taskName !== "D1 for MMT request" && (
              <PopupPlaceholder
                ...
                onMarkComplete={() => {
                  markTaskComplete(
                    popupContext.milestoneIndex,
                    popupContext.taskName,
                  );
                  closePopup();
                }}
              />
          ...
          {popupContext.milestoneIndex === 1 &&
            popupContext.taskName === "meeting completed" && (
              <PopupMeetingCompleted
                ...
                onClose={closePopup}
                onShareMom={() => {
                  markTaskComplete(1, "meeting completed");
                  closePopup();
                }}
              />
            )}
          {popupContext.milestoneIndex === 1 &&
            popupContext.taskName === "Design finalisation meeting request" && (
              <PopupPlaceholder
                message="Design finalisation meeting request"
                onMarkComplete={() => {
                  markTaskComplete(1, "Design finalisation meeting request");
                  closePopup();
                }}
              />
            )}
          {popupContext.milestoneIndex === 1 &&
            popupContext.taskName === "DQC 1 submission - dwg + quotation" && (
              <PopupPlaceholder
                message="Hi from DQC 1 submission - dwg + quotation"
                onMarkComplete={() => {
                  markTaskComplete(1, "DQC 1 submission - dwg + quotation");
                  closePopup();
                }}
              />
            )}
          {popupContext.milestoneIndex === 2 &&
            popupContext.taskName === "10% payment collection" && (
              <PopupPlaceholder
                message="10% payment collection"
                onMarkComplete={() => {
                  markTaskComplete(2, "10% payment collection");
                  closePopup();
                }}
              />
            )}
          {popupContext.milestoneIndex === 2 &&
            popupContext.taskName === "10% payment approval" && (
              <PopupPlaceholder
                message="10% payment approval"
                onMarkComplete={() => {
                  markTaskComplete(2, "10% payment approval");
                  closePopup();
                }}
              />
            )}
          {popupContext.milestoneIndex === 1 &&
            popupContext.taskName === "DQC 1 approval" && (
              <PopupDqc1Approval
                onClose={closePopup}
          ...
          {popupContext.milestoneIndex === 3 &&
            popupContext.taskName !== "D2 - masking request raise" && (
              <PopupPlaceholder
                message={popupContext.taskName}
                onMarkComplete={() => {
                  markTaskComplete(3, popupContext.taskName);
                  closePopup();
                }}
              />
            )}
          {popupContext.milestoneIndex === 4 && (
            <PopupPlaceholder
              message={popupContext.taskName}
              onMarkComplete={() => {
                markTaskComplete(4, popupContext.taskName);
                closePopup();
              }}
            />
          )}
          ...
          {popupContext.milestoneIndex === 5 &&
            popupContext.taskName !== "Design sign off" && (
              <PopupPlaceholder
                message={popupContext.taskName}
                onMarkComplete={() => {
                  markTaskComplete(5, popupContext.taskName);
                  closePopup();
                }}
              />
            )}
          {popupContext.milestoneIndex === 6 && (
            <PopupMeetingCompleted
              ...
              onClose={closePopup}
              onShareMom={() => {
                markTaskComplete(6, popupContext.taskName);
                closePopup();
              }}
            />
          )}
      ...
      {/* separate modal for checklist triggered via three‑dot menu */}
      {checklistContext && (
        <TaskModal
          context={{
            milestoneIndex: checklistContext.milestoneIndex,
            milestoneName:
              MileStonesArray.MilestonesName[checklistContext.milestoneIndex]
                .name,
            taskName: checklistContext.taskName,
          }}
          onClose={() => setChecklistContext(null)}
        >
          {activeChecklistDefinition && (
            <GenericMeetingChecklistPopup
              milestoneIndex={checklistContext.milestoneIndex}
              taskName={checklistContext.taskName}
